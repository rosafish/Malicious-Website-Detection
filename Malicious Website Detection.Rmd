...
website <- website %>%
  mutate(WHOIS_REGDATE = as.Date(WHOIS_REGDATE, format = ifelse(grepl("T", WHOIS_REGDATE), "%Y-%m-%dT", "%d/%m/%Y"))) %>% 
  ...
  mutate(WHOIS_REGDATE = impute(WHOIS_REGDATE, mean))
...
website <- website %>%
  ...
  mutate(WHOIS_UPDATED_DATE = impute(WHOIS_UPDATED_DATE, mean))
# factor Type
...
regtest <- website %>% mutate(WHOIS_REGDATE = impute(WHOIS_REGDATE, mean))
gf_boxplot(WHOIS_REGDATE ~ Type, data = regtest)
```
...
```{r}
# glm2
glm.full <- glm(Type ~ . , data = website.train, family = binomial)
summary(glm.quant)
