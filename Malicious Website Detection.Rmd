...
# No packets
pred.mod.no.packet <- pred.mod.full
summary(pred.mod.no.packet)

# Avoid overfitting, use the reduced model we got from above
...
# No server
pred.mod.no.server <- update(inf.mod.aic, . ~ . -SERVER, data = website.train, family = binomial)
summary(pred.mod.no.server)

# No country
pred.mod.no.country <- update(inf.mod.aic, . ~ . -WHOIS_COUNTRY, data = website.train, family = binomial)
...
```{r}
pred.mod.no.packet.sum <- summary(pred.mod.no.packet)
pander(cbind(pred.mod.no.packet.sum$coefficients), type = "pdf")
```
